...
export function createScene() {
  ...
  // Environment map (HDR with LDR fallback), applied to scene.environment only
  try {
    const pmrem = new THREE.PMREMGenerator(renderer);
    pmrem.compileEquirectangularShader?.();
    const hdrUrl = 'https://threejs.org/examples/textures/equirectangular/royal_esplanade_1k.hdr';
    const ldrUrl = 'https://threejs.org/examples/textures/equirectangular/royal_esplanade_1k.jpg';
    const applyEnv = (tex) => {
      try {
        const envRT = pmrem.fromEquirectangular(tex);
        const envMap = envRT.texture;
        scene.environment = envMap;
        // Kun environment valmis, leikataan sen intensiteetti varjojen kontrastin parantamiseksi
    ...
    new RGBELoader()
      ...
      .load(hdrUrl, (hdrTex) => {
        ...
        new THREE.TextureLoader().load(ldrUrl, (jpgTex) => {
          jpgTex.mapping = THREE.EquirectangularReflectionMapping;
          jpgTex.colorSpace = THREE.SRGBColorSpace;
          applyEnv(jpgTex);
        }, undefined, () => { try { pmrem.dispose(); } catch {} });
      });
  } catch {}
